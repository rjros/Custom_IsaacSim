...
class DeformableBodyDemo(demo.Base):
    ...
    description = "This snippet sets up a deformable cube scene"

    @staticmethod
    def create_cube_mesh(stage, target_path):
        # Create a cube primitive instead of a sphere
        _, tmp_path = omni.kit.commands.execute(
            "CreateMeshPrim", prim_type="Cube", select_new_prim=False
        )
    ...
    def create(self, stage):
        ...
        # Create cube mesh used as the 'skin mesh' for the deformable body
        skin_mesh = self.create_cube_mesh(stage, default_prim_path + "/deformableBody")

        # Position and scale cube
        skin_mesh.GetPrim().GetAttribute("xformOp:translate").Set(Gf.Vec3f(0.0, 0.0, 300.0))
        skin_mesh.GetPrim().GetAttribute("xformOp:scale").Set(Gf.Vec3f(2.0, 2.0, 2.0))

        # Optional coloring
        color = demo.get_primary_color()
        skin_mesh.CreateDisplayColorAttr().Set([color])

        # Simulation resolution for hexahedral generation
        simulation_resolution = 10

        # Apply deformable body API to cube
        ...
        deformable_material_path = omni.usd.get_stage_next_free_path(
            stage, default_prim_path + "/deformableBodyMaterial", True
        )
